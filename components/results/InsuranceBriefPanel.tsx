...
export function ReadinessVerdictPanel({
  ...
  return (
    <div className={`rounded-sm border p-4 sm:p-5 ${STATUS_PANEL[readiness.statusColor]}`}>
      ...
      {missingEvidenceCount !== undefined && missingEvidenceCount > 0 && (
        <p className="mt-4 text-sm text-slate-600">
          <span className="font-semibold">{missingEvidenceCount}</span> expected evidence item
          {missingEvidenceCount !== 1 ? "s are" : " is"} missing — see the Evidence Checklist
          below for what must be fixed first.
        </p>
      )}
    </div>
  );
...
export function BrokerSummaryPanel({ brief }: { brief: InsuranceBrief }) {
  const { brokerSummary, underwriterNotes } = brief;

  return (
    <div className="space-y-5">
      {brokerSummary.map((section) => (
